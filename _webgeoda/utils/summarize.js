...
export const parseTooltipData = (geoid, state, presets) => {
  let tooltipData = [];
  const variables = presets?.style?.tooltip?.displayOnlyCurrentVariable 
    ? [presets.variables.find(f => f.variable === state.dataParams.variable)]
    : presets.variables;
  ...
  for (let i = 0; i < variables.length; i++) {
    ...
    parsedVariable !== null && tooltipData.push({
