...
## MK note -- Mention here that "shapefiles" are considered standard spatial formats, that have both a spatial and flat-file data information. For spatial newbies.

```{r read zips}
il_zips <- st_read("il_zips.shp")
```

## MK note -- show that zip codes have data attached them, too; would add head() to see that info. This is what they'll want in the end -- a CSV file with zip code and minimum distance value. 
...
## Mk Note -- You may also want to save as CSV file here, for those who are just trying to get it back into that format. That's usually the top request; give me a variable to link back to my CSV file.


## Visualize with CARTO (separate tutorial - maybe not in R Markdown?)
## MK note -- I would visualize using tmap in R. We may show the data ourselves using CARTO links, but tutorial may not be necessary just yet.

## Make into Shiny app (separate tutorial)
## MK note -- May not be necessary for this crowd.
