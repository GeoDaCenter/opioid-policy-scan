...
# Date: 8/7/2023
# About: This R script combines the 1980 historic data on 2010 geometries, and aggregates
...
ds01 <- read.csv('../data_final/DS01_T_1980_NHGIS.csv') |>
  mutate(GEOID = str_pad(FIPS, width=11, side='left', pad='0')) |>
  select(-FIPS)

# These two datasets were collected by Social Explorer. They feature data for
# every 2010 census tract. However, the 1980 census did not collect that much
# data -- they only collected 59187 tracts worth of data -- we discard the
# excess data.

ec03 <- read.csv('../data_final/EC03_T_1980_DC.csv') |>
  mutate(GEOID = str_pad(FIPS, width=11, side='left', pad='0')) |>
  select(-FIPS, -X, -NAME, -year)
hs01 <- read.csv('../data_final/HS01_T_1980_DC.csv') |>
  mutate(GEOID = str_pad(FIPS, width=11, side='left', pad='0')) |>
  select(-FIPS, -X, -NAME, -year)

# 1980 census tracts featured suppression of age data in small census tracts.
# Thus, the totalPop variable has more people than the list of people whose
# age was actually counted. 

# The upshot of this is that calculating ChildrenP requires us to make a new
# variable called "totalPop.age" which is just the number of people whose 
# age was recorded.

totalPop.age <- ds01$age0_4   + ds01$age5_14  + ds01$age15_44 + 
                ds01$age45_54 + ds01$age55_59 + ds01$age60_64 + 
                ds01$ageOv65

ds01 <- ds01 |> 
  mutate(childrenP = round(100 * (totalPop.age - age18_64 - ageOv65) / totalPop.age, 2))
...
tracts <- merge(ds01, ec03, by='GEOID') |>
  ...
  select( 
         ...
         age5_14,  age15_19, age20_24, age15_44,  age45_54, 
         age55_59, age60_64, ageOv65,  Age15_24P, Und45P, 
         Ovr65P,   WhiteP,   BlackP,   HispP,     AmIndP,
         AsianP,   PacIsP,   OtherP,   NoHSP,     childrenP, 
         povP,     UnempP,   occP,     vacantP
  )

names <- read.csv('../data_raw/rename_tables/1980.csv')
...
write.csv(tracts, "../data_final/consolidated/T_1980.csv", row.names=F)

#### Counties ---- 

ds01 <- read.csv('../data_final/DS01_C_1980_NHGIS.csv') |>
  mutate(GEOID = str_pad(GEOID, width=5, side='left', pad='0'),
         childrenP = round(100 * (totPop - age18_64 - ageOv65) / totPop, 2) )

ec03 <- read.csv('../data_final/EC03_C_1980_DC.csv') |>
  mutate(GEOID = str_pad(FIPS, width=5, side='left', pad='0')) |>
  select(-FIPS, -NAME, -year)
hs01 <- read.csv('../data_final/HS01_C_1980_DC.csv') |>
  mutate(GEOID = str_pad(FIPS, width=5, side='left', pad='0')) |>
...
cntySave |> write.csv('C_1980.csv', row.names=F)
...
states |> write.csv("../data_final/consolidated/S_1980.csv", row.names = F)
