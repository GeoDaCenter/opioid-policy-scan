...
tracts <- merge(ds01, ec03, by='GEOID') |>
  ...
  select( 
    ...
    AmIndP,    AsianP,    PacIsP,   OtherP,   DisbP,
    NoHSP,     childrenP, povP,     UnempP,   occP,      
    vacantP
  )

## Fix names

names <- read.csv('../data_raw/rename_tables/2000.csv')
...
write.csv(tracts, "../data_final/consolidated/T_2000.csv", row.names=F)
rm(tracts)
...
## Subset a portion of the data for saving.
...
cntySave |> write.csv('../data_final/consolidated/C_2000.csv', row.names=F)
...
states <- cnty |> mutate(
  ...
  summarize(
    ...
    NoHSPop  = sum(edSmpl * NoHSP,            na.rm = T),
    edSmpl   = sum(edSmpl,                    na.rm = T),
    ...
    DisbPop  = sum(DisbP * (totPop - age0_4), na.rm = T)
  ) #|>
  mutate(
    ...
    DisbP     = round(DisbPop / (TotPop - Age0_4), 2)
  ) |>
  select(
...
states |> write.csv("../data_final/consolidated/S_2000.csv", row.names=F)
