...
tracts <- read.csv('../data_raw/nhgis/1990DataInterpolated.csv') |>
  mutate(GEOID  = str_pad(FIPS, width=11, side='left', pad='0'),
         income = perCapInc * totPop)

totUnits <- read.csv('../data_final/consolidated/T_1990.csv') |>
  mutate(GEOID   = str_pad(GEOID, width=11, side='left', pad='0'),
         VacUnit = VacantP * TotUnits) |>
  select(GEOID, TotUnits, VacUnit)

pov_lab <- read.csv('../data_final/EC03_T_1990_DC.csv') |>
  mutate(GEOID = str_pad(FIPS, width=11, side='left', pad='0')) |>
  select(GEOID, Unemp, labor, pov, povUni)
  ...
  if (has_matches) {
    ...
    for (relevant_row_num in 1:nrow(relevant_rows)) {
      ...
      zips_2010[zips_row, ] <- c(
        ...
        OtherPop  = zips_2010[ zips_row, 'OtherPop'] + res_ratio * tracts[ tract_row, 'otherPop'        ],
        NoHsPop   = zips_2010[ zips_row, 'NoHsPop' ] + res_ratio * tracts[ tract_row, 'noHighSchoolDeg' ],
        EdSampl   = zips_2010[ zips_row, 'EdSampl' ] + res_ratio * tracts[ tract_row, 'edSampl'         ],
        povPop    = zips_2010[ zips_row, 'povPop'  ] + res_ratio * tracts[ tract_row, 'pov'             ],
        povUni    = zips_2010[ zips_row, 'povUni'  ] + res_ratio * tracts[ tract_row, 'povUni'          ],
        unempPop  = zips_2010[ zips_row, 'unempPop'] + res_ratio * tracts[ tract_row, 'Unemp'           ],
...
zips_2010s <- zips_2010 |>
  mutate(
    ...
    OtherP    = round(100 * OtherPop / TotPop,  2),
...
write.csv(zips_2010s, '../data_final/consolidated/Z_1990.csv', row.names = F)
